...
class NN_jackal():
    def __init__(self, veh_name, veh_data, nn, actions):
        ...
        self.operation_mode = True #PlannerMode()
        ...
        self.pub_twist = rospy.Publisher('/jackal_velocity_controller/cmd_vel',Twist,queue_size=1)
        ...
        self.sub_pose = rospy.Subscriber('/nn_jackal/pose',PoseStamped,self.cbPose)
        self.sub_vel = rospy.Subscriber('/nn_jackal/velocity',Vector3,self.cbVel)
        ...
        self.sub_global_goal = rospy.Subscriber('/nn_jackal/goal',PoseStamped, self.cbGlobalGoal)
    ...
    def cbClusters(self, msg):
        ...
        radii = [];
        labels = []
        num_clusters = len(msg.lmpcc_obstacles)
        for obs in msg.lmpcc_obstacles:
            index = obs.id
            # index = msg.labels[i]
            x = obs.pose.position.x
            y = obs.pose.position.y
            v_x = obs.velocity.linear.x
            v_y = obs.velocity.linear.y
            # radius = PED_RADIUS
    ...
    def cbControl(self, event):
        #print("cbControl")
        # self.goal.header.stamp == rospy.Time(0) or
        if self.stop_moving_flag and not self.new_global_goal_received:
            self.stop_moving()
            return
        elif True:#self.operation_mode.mode==self.operation_mode.NN:
